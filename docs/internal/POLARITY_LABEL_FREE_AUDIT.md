...
**`eval_roc_auc` / `eval_average_precision`：** 仅在 `model.py` 指标段与可选 `FMGAD_POLARITY_DEBUG` 调试打印中出现；**未**用于 checkpoint 选择、超参搜索或极性分支（`run_ablation.py` 的 `max(auc,...)` 仅为静态字符串黑名单，非运行时代码）。
...
- `configs/*.yaml`（各数据集）、`configs/universal_template.yaml`：`polarity_adapter: universal_no_y`。
- `scripts/dev/run_universal_5x5_sweep.py`、`scripts/dev/run_universal_param_tune.py`、`scripts/run_ablation.py`：适配 `universal_no_y`。
